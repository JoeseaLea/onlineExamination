...
<html>
  ...
  <table cellSpacing=0 cellPadding=0 width="1180px" style="margin-left: -10px; margin-right: -5px; margin-top: -5px; border: 0">
    ...
    <table border="1" bordercolor="#C6E6FF" cellspacing="0">
    	...
    	<c:forEach items="${listQuestionBankJudgeBean }" var="questionBankJudgeBean">
    	<tr>
    		<td>${questionBankJudgeBean.teachername }</td>
    		<td>
    			<c:forEach items="${listCourseBean }" var="courseBean">
    				<c:if test="${questionBankJudgeBean.courseid==courseBean.id }">${courseBean.name }</c:if>
    			</c:forEach>
    		</td>
    		<td>${questionBankJudgeBean.question }</td>
    		<td align="center">
    			<c:if test="${questionBankJudgeBean.answer==1 }">对</c:if>
    			<c:if test="${questionBankJudgeBean.answer==2 }">错</c:if>
    		</td>
    		<td align="center"><a href="findQuestionBankJudgeByIdAction?questionBankJudgeBean.id=${questionBankJudgeBean.id }">更新</a></td>
    		<td align="center"><a href="deleteQuestionBankJudgeAction?questionBankJudgeBean.id=${questionBankJudgeBean.id }">删除</a></td>
    	</tr>
    	</c:forEach>
    </table>
   	</center>
